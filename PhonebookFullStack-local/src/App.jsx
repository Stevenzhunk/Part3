...
const PersonForm = ({handleSumit,newName,handleChange,newPhone,handlePhoneChange})=>{
  return(
...
const Numbers = ({showPerson,personDelete,updatePerson}) =>{
  ...
  return(
    ...
    <div key={person.id}>
      ...
      <button
      onClick={()=>handleOnclickNumber(person)}>Delete</button>
      </div>
    ))}
    </div>
  )
...
const App = () => {
  ...
  const checkExistence = (array, name) => array.some(obj => obj.name === name);
  ...
  const handleSumit =(event) =>{
    ...
    if (result){
      ...
      confirm(`the user ${newName} is already added to phonebook you wanna udapte number?`)
      ...
      .then (response => {updatePerson(response)
        SetSelector(true)
        SetNewMessage(
      ...
      .catch( error=> {
        SetSelector(false)
        SetNewMessage(
          `the number of ${newUpdate.name} was already Delete`
        )
        setTimeout(() => {
          SetNewMessage(null)
        }, 4000)
  ...
  const showPerson = (newfilter==='')
  ? persons
  : finderPerson(persons,newfilter);
